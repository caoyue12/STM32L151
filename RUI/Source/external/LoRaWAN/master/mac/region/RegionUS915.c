        ...
        for( uint8_t j = 0; j < 16; j++ )
        {
            if( ( channelsMask[k] & ( 1 << j ) ) != 0 )
            {
                ...
                { // Check if the channel is enabled
						...
						e_printf(" channel[%d] is disabled\r\n",i + j);
#endif					
                    continue;
                }
...
#if DEBUG_FW_JOINEDTIMER
						e_printf(" channel[%d] isn't supports the given datarate\r\n",i + j);
#endif	
                    continue;
                }
...
#if DEBUG_FW_JOINEDTIMER
						e_printf(" channel[%d]Band].TimeOff =%d\r\n",i + j,bands[channels[i + j].Band].TimeOff);
#endif	
                    delayTransmission++;
                    continue;
                }
#if 0 
                e_printf("enabledChannels[%i]\r\n", i + j);
#endif				
...
//    e_printf("\r\nFreq=%d, txP=%d dbm, SF%d ,tm=%d,DR=%d\r\n", Channels[txConfig->Channel].Frequency, phyTxPower, phyDr, *txTimeOnAir,txConfig->Datarate);
    return true;
}
...
int8_t RegionUS915AlternateDr( AlternateDrParams_t* alternateDr )
{
    ...
    if( ( alternateDr->NbTrials & 0x01 ) == 0x0 )
    {
        datarate =DR_2; //DR_4;
    }
    else
