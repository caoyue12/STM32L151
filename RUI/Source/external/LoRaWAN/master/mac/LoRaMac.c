...
static E_LORA_MODE  g_lora_mode = LORAWAN_MODE;  //0: LoraWAN mode 1: LoraP2P mode 2:LoraCW mode
...
#if DEBUG_FW
//		e_printf("OnMacStateCheckTimerEvent !\r\n");
#endif
...
#if DEBUG_FW
    e_printf("\r\nRx1_Freq=%d, Dr_tb[%d], RxSlot=%d\r\n", RxWindow1Config.Frequency, RxWindow1Config.Datarate,RxSlot);
#endif
}
...
//#if DEBUG_FW 		
//	e_printf("nextChan.AggrTimeOff=%d, nextChan.LastAggrTx=%d.\r\n",nextChan.AggrTimeOff,TimerGetElapsedTime(nextChan.LastAggrTx));		
//#endif
    // Select channel
    while( RegionNextChannel( LoRaMacRegion, &nextChan, &Channel, &dutyCycleTimeOff, &AggregatedTimeOff ) == false )
    {
#if DEBUG_FW 		
	e_printf("Set the default datarate.\r\n");		
#endif
...
//#if DEBUG_FW
//	e_printf("AggregatedTimeOff=%d\r\n TxTimeOnAir=%d,AggregatedDCycle=%d\r\n",AggregatedTimeOff,TxTimeOnAir,AggregatedDCycle);
//#endif
}
...
#if DEBUG_FW 	
e_printf("Freq=%d, SF%d, Cr=%d, Pr=%d, Txtime=%d\r\n", LoraP2PParams.Frequency, 
...
{
//#if DEBUG_FW 
//e_printf("SetTxContinuousWave1 \r\n");
//#endif
    Radio.SetTxContinuousWave( frequency, power, timeout );
//#if DEBUG_FW 
//e_printf("SetTxContinuousWave1 OK \r\n");
//#endif
    // Starts the MAC layer status check timer
    TimerSetValue( &MacStateCheckTimer, MAC_STATE_CHECK_TIMEOUT );
    TimerStart( &MacStateCheckTimer );
...
LoRaMacStatus_t LoRaMacMlmeRequest( MlmeReq_t *mlmeRequest )
{
    ...
    PhyParam_t phyParam;
#if DEBUG_FW
    e_printf("LoRaMacState=%d. MlmeReq_t=%d\r\n",LoRaMacState,mlmeRequest->Type);
#endif
    if( mlmeRequest == NULL )
    {
		e_printf("LoRaMac Status parameter invaild !\r\n");
        return LORAMAC_STATUS_PARAMETER_INVALID;
    }
    if( ( LoRaMacState & LORAMAC_TX_RUNNING ) == LORAMAC_TX_RUNNING )
    {
		e_printf("LoRaMac Status is busy now !\r\n");
        return LORAMAC_STATUS_BUSY;
    }
